...
from agents.graph import build_graph, build_intraday_graph, build_advanced_graph
...
from watchlist_manager import add_to_watchlist, remove_from_watchlist, get_watchlist
from dashboard_data_aggregator import get_watchlist_data
from alert_engine.alert_engine import add_alert, get_active_alerts, evaluate_alerts
from portfolio_engine.portfolio_manager import get_portfolio_summary
from execution_engine.performance_metrics import calculate_performance_metrics
from background_scheduler import start_alert_scheduler

st.set_page_config(page_title="TradeSentinel", layout="wide", initial_sidebar_state="expanded")
...
# Initialize Storage & Scheduler
init_db()
if "scheduler_started" not in st.session_state:
    st.session_state.scheduler_started = True
    start_alert_scheduler(interval_seconds=300)
...
st.markdown(
    ...
    f'{ICONS["shield"]}'
    f'<span style="font-size:1.7rem;font-weight:700;color:#E2E8F0;">TradeSentinel</span>'
    f'</div>',
    unsafe_allow_html=True
)
...
with tab1:
    ...
    if st.button("Run Analysis", type="primary"):
        with st.status("Initializing advanced workflow…", expanded=True) as status:
            workflow = build_advanced_graph()
...
# ══════════════════════════════════════════════════════════════
# TAB 5 · Watchlist Dashboard
# ══════════════════════════════════════════════════════════════
with tab2:
    st.markdown(f'{icon("list")} &nbsp;<strong style="font-size:1.15rem;">Watchlist Dashboard</strong>', unsafe_allow_html=True)
    st.caption("Live monitoring of your favorite stocks with automated RSI, Trend and Momentum analysis.")
    st.markdown("")
    
    col_add_w, col_list_w = st.columns([1, 3])
    with col_add_w:
        section_header("plus", "Quick Add")
        w_sym = st.text_input("Symbol", placeholder="RELIANCE.NS", key="w_add_input").upper()
        if st.button("Add to Watchlist"):
            if add_to_watchlist(w_sym):
    ...
    with col_list_w:
        section_header("activity", "Live Metrics")
        w_data = get_watchlist_data()
        if w_data:
            df_w = pd.DataFrame([d.model_dump() for d in w_data])
            st.dataframe(df_w, hide_index=True, use_container_width=True)
            
            # Action buttons for removal
            rm_w_sym = st.selectbox("Remove from watchlist", [d.symbol for d in w_data])
            if st.button("Delete from Watchlist"):
                remove_from_watchlist(rm_w_sym)
                st.rerun()
        else:
            st.info("Watchlist is empty. Add symbols to start monitoring.")

# ══════════════════════════════════════════════════════════════
# TAB 6 · Smart Alerts
# ══════════════════════════════════════════════════════════════
with tab5:
    st.markdown(f'{icon("zap")} &nbsp;<strong style="font-size:1.15rem;">Smart Alerts</strong>', unsafe_allow_html=True)
    st.caption("Automated rule-based alerts. No LLMs required – 100% local rule execution.")
    st.markdown("")
    
    col_a1, col_a2 = st.columns([1, 2])
    with col_a1:
        section_header("plus", "Create Alert")
        a_sym = st.selectbox("Symbol", get_watchlist())
        a_cond = st.selectbox("Condition", ["price_above", "price_below", "rsi_above", "rsi_below", "volume_spike"])
        a_val = st.number_input("Value / Threshold", value=0.0)
        if st.button("Set Alert"):
            add_alert(a_sym, a_cond, a_val)
            st.success(f"Alert set for {a_sym}!")
            st.rerun()
            
    with col_a2:
        section_header("clock", "Active Alerts")
        active_a = get_active_alerts()
        if active_a:
            st.table(active_a)
        else:
            st.caption("No active alerts.")
            
        if st.button("Run Manual Alert Check"):
            triggered = evaluate_alerts()
            if triggered:
                for t in triggered:
                    st.toast(f"🚨 ALERT TRIGGERED: {t['symbol']} {t['condition']} {t['current_value']:.2f}")
            else:
                st.write("No alerts triggered.")

# ══════════════════════════════════════════════════════════════
# TAB 7 · Paper Trading (Auto-Sim)
# ══════════════════════════════════════════════════════════════
with tab6:
    st.markdown(f'{icon("shield")} &nbsp;<strong style="font-size:1.15rem;">Paper Trading Simulator</strong>', unsafe_allow_html=True)
    st.caption("Real-time auto-trading simulation based on local technical rules.")
    st.markdown("")
    
    metrics = calculate_performance_metrics()
    m_col1, m_col2, m_col3, m_col4 = st.columns(4)
    m_col1.metric("Win Rate", metrics["win_rate"])
    m_col2.metric("Total PnL", metrics["total_pnl"])
    m_col3.metric("PnL %", metrics["pnl_pct"])
    m_col4.metric("Available Cash", metrics["current_cash"])
    
    st.markdown('<hr class="tg-divider">', unsafe_allow_html=True)
    
    sc1, sc2 = st.columns(2)
    with sc1:
        section_header("activity", "Current Virtual Positions")
        p_summary = get_portfolio_summary()
        if p_summary["positions"]:
            st.dataframe(p_summary["positions"], use_container_width=True)
        else:
            st.caption("No open positions.")
            
    with sc2:
        section_header("check", "Auto-Trade Control")
        if st.button("Trigger Auto-Sim Sync"):
            print("hey im triggered auto sim sync")
            from execution_engine.simulation_engine import run_simulation_step
            with st.spinner("Analyzing market and executing pending trades..."):
                run_simulation_step()
